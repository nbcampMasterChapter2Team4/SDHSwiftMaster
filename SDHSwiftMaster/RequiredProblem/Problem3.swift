...
//        print(d(["가", "나", "다", "라", "마"]))  // Error: String 타입은 Numeric 프로토콜을 따르지 않음
    ...
    // - 테스트 입력: [1, 2, 3, 4, 5]
    // - 테스트 출력: [1, 3, 5]
    private static func a(_ input: [Int]) -> [Int] {
        var output: [Int] = []
    ...
    // - 테스트 입력: ["가", "나", "다", "라", "마"]
    // - 테스트 출력: ["가", "다", "마"]
    private static func b(_ input: [String]) -> [String] {
        var output: [String] = []
    ...
    // 제네릭
    private static func c<T>(inputGeneric: [T]) -> [T] {
    ...
    // Any + 가변 매개변수
    private static func c(inputsAny: [Any]...) -> [[Any]] {
        var outputs: [[Any]] = []
    ...
    private static func d<T: Numeric>(_ input: [T]) -> [T] {
        let output = input.enumerated()
            .filter { $0.offset % 2 == 0 }
            .map { $0.element }
